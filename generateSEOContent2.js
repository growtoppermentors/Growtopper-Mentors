...
let blogsStr = fs.readFileSync(path.join(__dirname, 'src/data/blogs.js'), 'utf-8');
...
// Manually replace content block for first 10
const updatedStr = blogsStr.replace(/title:\s*["']([^"']+)["'][\s\S]*?content:\s*\`([\s\S]*?)\`/g, (match, title, oldContent) => {
  let cleanIntro = "<p>" + oldContent.replace(/<[^>]+>/g, '').substring(0, 300) + "...</p>";
...
fs.writeFileSync(path.join(__dirname, 'src/data/blogs.js'), updatedStr);
console.log('Done mapping blogs.js!');
